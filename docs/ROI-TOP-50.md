...
**50. Atomic power-loss-resilient header writes** `[M][FORMAT]` — 06-27 — **DONE (torn-write recovery proven; true power-loss = real-build)** (design reviewed → 8-byte gen, keyslot-area A/B rides the existing backup-header mirroring). `Common/AtomicHeader.{c,h}` (gated `VC_ENABLE_ATOMIC_HEADER`) treats the primary+backup header pair as an A/B slot with a monotonic 8-byte generation and a commit tag: `committed copy = header || gen[8 BE] || HMAC-SHA256(K, header||gen)`. Write the inactive (older/invalid) copy fully → fsync → it becomes newest by gen; mount picks the valid copy with the greatest gen, falling back if the newest is torn, and fails closed if neither is valid. Suite step `[77]` verifies two ways: the commit-tag layout byte-for-byte vs `atomic_header_reference.py` (anchor `c6f46900…`), plus a **torn-write simulation** over an in-memory medium — for every split offset the in-flight copy is `new[0:k] || old[k:]`, and `AtomicHeaderSelect` must always return a valid committed header (553 offsets: the fully-written one recovers the new gen-3 header, all 552 torn offsets fall back to the previous committed header, `bad=0`). Negative controls: corrupting the chosen copy forces fallback to the other; corrupting **both** fails closed (`-1`, refuse to mount stale/garbage). Honest limit: true power-loss recovery needs real hardware / a fault-injection block device (not sandbox-testable); same-generation rollback needs tamper-resistant monotonic storage. gcc-13 + clang-18; in the flag matrix. `patches/atomic-header.patch` applies -p1. (The `fsync`-ordered write path in the volume I/O layer and the keyslot-area double-buffering on the backup-header mirror are the remaining real-build wiring.)
