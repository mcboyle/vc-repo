...
| Rank-1 HKDF migration (v2 derivation) + mount-time version-try loop | **BUILT + WIRED** behind `VC_ENABLE_HKF_MIX_V2` (`HKFMixResponseIntoPasswordV2` / `…Ver`, `HKFComputeActiveResponse`, `HKFApplyIfConfiguredVer`); core verified at step `[80]`, mount/create **wiring** (both paths, compute-once, cross-path identity) at step `[81]` (`hkf_mixv2_wiring_test.c`). Behavioural header round-trip is real-build-only. | `src/Common/HardwareKeyFactor.c`, `src/Common/Volumes.c`, `src/Volume/VolumeHeader.cpp`, `src/Core/VolumeCreator.cpp`, `src/Volume/HardwareKeyFactorMix.h`, `docs/HKF-MIX-V2-SPEC.md` |
