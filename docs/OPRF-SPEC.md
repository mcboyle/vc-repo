...
**Status: protocol proven; server + integration are real-build.** An Oblivious PRF (2HashDH / CFRG
DH-OPRF) makes the derived key depend on both the **password** and a **secret held by a rate-limited
server**, while the server learns neither the password nor the output. Consequence: a **stolen disk
...
- **Real group.** Use a CFRG prime-order group — **ristretto255** or NIST P-256 — via a real EC library
  (or hybridize, see below), never the PoC field. The construction is exactly the CFRG OPRF draft.
