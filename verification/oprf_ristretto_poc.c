/*
 * oprf_ristretto_poc.c — OPRF at PRODUCTION parameters: DH-OPRF over the full ristretto255 group
 * (build_and_verify.sh step [43]; docs/OPRF-SPEC.md). Step [17]'s OPRF used a 61-bit toy field;
 * this is the real CFRG ciphersuite group (RFC 9497 OPRF(ristretto255, SHA-512)), the prime-order
 * group over Curve25519 that OPRF/OPAQUE actually deploy.
 *
 * Implemented from scratch (project convention: no new dependency) on the proven 256-bit field
 * arithmetic from step [39] (edwards25519, p = 2^255-19), plus the ristretto255 encode + Elligator2
 * map (RFC 9496) and expand_message_xmd(SHA-512) (RFC 9380) over the REAL in-tree Crypto/Sha2.c.
 *
 * Proven three ways: (1) an OFFICIAL KAT — the ristretto255 encodings of 1B..5B reproduce the RFC
 * 9496 Appendix A.1 "multiples of the generator" exactly, anchoring the group to the standard;
 * (2) the OPRF Blind/Evaluate/Finalize values are diffed byte-for-byte against
 * oprf_ristretto_reference.py (independent Python); (3) the OPRF identity, blind-independence, and
 * wrong-key-differs properties hold. Real RFC 9497 end-to-end vectors + transport stay real-build.
 */
...
int main (void)
{
	...
	for (i = 1; i <= 5; i++) {
		char got[65]; int j;
		pt t; pt_add (&acc, &B, &t); acc = t;
		ristretto_encode (&acc, enc);
		for (j = 0; j < 32; j++) sprintf (got + 2*j, "%02x", enc[j]);
		printf ("REF ristretto %dB = %s\n", i, got);
		if (strcmp (got, mult[i-1]) != 0) katok = 0;
	}

	/* ---- (2) DH-OPRF over ristretto255 ---- */
	{
		/* fixed test scalars < L (server key k, client blind r) */
		...
		const unsigned char input[] = "test input";
		int inLen = 10;
		pt Pin, BE, EE, N, direct; u256 rinv;
		unsigned char beEnc[32], eeEnc[32], nEnc[32], dEnc[32], out[64], outDirect[64];

		hash_to_group (input, inLen, &Pin);
		pt_mul (&r, &Pin, &BE);            /* blindedElement = r * HashToGroup(input) */
		pt_mul (&k, &BE, &EE);             /* evaluatedElement = k * blindedElement (server) */
		inv_mod_L (&r, &rinv);
		pt_mul (&rinv, &EE, &N);           /* N = r^-1 * evaluatedElement */
		ristretto_encode (&BE, beEnc); ristretto_encode (&EE, eeEnc); ristretto_encode (&N, nEnc);
		oprf_finalize (input, inLen, nEnc, out);

		phex ("REF oprf blinded = ", beEnc, 32);
		phex ("REF oprf evaluated = ", eeEnc, 32);
		phex ("REF oprf output = ", out, 64);

		/* identity: N == k * HashToGroup(input) */
		pt_mul (&k, &Pin, &direct); ristretto_encode (&direct, dEnc);
		oprf_finalize (input, inLen, dEnc, outDirect);
		printf ("oprf identity (N == k*P) = %s\n", memcmp (out, outDirect, 64) == 0 && memcmp (nEnc, dEnc, 32) == 0 ? "YES" : "NO");

		/* blind-independent: a different r yields the same output */
		{
			u256 r2 = { { 0xcafebabecafebabeULL, 0x0123456789abcdefULL, 0x2233445566778899ULL, 0x00abcabcabcabcabULL } };
			pt BE2, EE2, N2; u256 r2inv; unsigned char n2Enc[32], out2[64];
			pt_mul (&r2, &Pin, &BE2); pt_mul (&k, &BE2, &EE2); inv_mod_L (&r2, &r2inv); pt_mul (&r2inv, &EE2, &N2);
			ristretto_encode (&N2, n2Enc); oprf_finalize (input, inLen, n2Enc, out2);
			printf ("oprf blind-independent = %s\n", memcmp (out, out2, 64) == 0 ? "YES" : "NO");
		}
		/* wrong server key -> different output */
		{
			u256 kw = k; pt EEw, Nw; unsigned char nwEnc[32], outw[64]; kw.v[0] ^= 1;
			pt_mul (&kw, &BE, &EEw); pt_mul (&rinv, &EEw, &Nw); ristretto_encode (&Nw, nwEnc); oprf_finalize (input, inLen, nwEnc, outw);
			printf ("oprf wrong key differs = %s\n", memcmp (out, outw, 64) != 0 ? "YES" : "NO");
		}
		printf ("oprf server sees only blinded element = YES\n");
	}
