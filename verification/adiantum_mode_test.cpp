/*
 * adiantum_mode_test.cpp — module test for the EncryptionMode shim, src/Volume/EncryptionModeAdiantum.
 *
 * WHAT THIS ADDS THAT [91] DOES NOT. Step [91] proves the Adiantum ALGORITHM against all 18 official
 * google/adiantum KATs. It says nothing about the EncryptionMode subclass, which is where the shim can
 ...
 * Adiantum over XTS in this fork, and it is exactly the property a shim can silently lose (e.g. by
 * calling the primitive per 16-byte block instead of per sector). Under XTS, flipping one plaintext
 * byte changes ONE 16-byte ciphertext block; under a correct Adiantum sector mode it must randomise the
...
static shared_ptr <EncryptionModeAdiantum> MakeMode (uint8 keyByte)
{
	shared_ptr <EncryptionModeAdiantum> m (new EncryptionModeAdiantum);
	uint8 key[EncryptionModeAdiantum::AdiantumKeySize];
...
int main ()
{
	printf ("  EncryptionModeAdiantum shim test (src/Volume/EncryptionModeAdiantum.cpp)\n");

	const size_t SEC = 512;
	shared_ptr <EncryptionModeAdiantum> mode = MakeMode (0x11);

	check ("key is set after SetKey", mode->IsKeySet());
	check ("GetKeySize() == 32 (Adiantum's own key, not a sum over Ciphers)", mode->GetKeySize() == 32);
	check ("GetName() == \"Adiantum\"", mode->GetName() == L"Adiantum");
	...
	{
		uint8 x[SEC], y[SEC];
		shared_ptr <EncryptionModeAdiantum> shifted = MakeMode (0x11);
	...
	{
		uint8 k1[SEC], k2[SEC];
		shared_ptr <EncryptionModeAdiantum> other = MakeMode (0x77);
	...
	printf ("  [6] buffer path + partial-unit refusal\n");
	{
		...
		try { mode->EncryptSectorsCurrentThread (buf, 0, 1, ADIANTUM_MAX_SECTOR + 512); }
	...
	printf ("  ADIANTUM MODE: %d passed, %d failed\n", pass, fail);
	if (fail) { printf ("  ADIANTUM MODE SHIM TEST FAILED\n"); return 1; }
	printf ("  ADIANTUM MODE SHIM TEST PASSED\n");
	return 0;
}
