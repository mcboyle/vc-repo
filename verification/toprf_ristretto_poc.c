 ...
 * of the server key over the scalar field Z_L, so no single server holds k.
 *
 * The server key k is Shamir-split into k_1..k_n over Z_L (prime field, degree t-1). Each server i
 * returns a partial Z_i = k_i * blindedElement; the client combines any t of them by
 * Lagrange-in-the-exponent: sum_{i in S} lambda_i * Z_i = (sum lambda_i k_i) * BE = k * BE, so t
 * partials reconstruct exactly the single-key OPRF output, t-1 learn nothing, and no server sees the
 * password or the unblinded element. Group + encode reuse the step-[43] code (RFC 9496/9497), over
 * the REAL in-tree Sha2.c.
 *
 * Proven: (1) the RFC 9496 basepoint-multiples KAT anchors the group; (2) the threshold output is
 * diffed byte-for-byte vs toprf_ristretto_reference.py; (3) any-t-reconstructs-single,
 * any-t-subset-agrees, below-threshold-differs, server-oblivious. Transport/server stay real-build.
 */
...
/* ---- scalar field Z_L (the ristretto255 group order) ---- */
static void sc_mul (const u256 *a, const u256 *b, u256 *r) { mulmod (a, b, &L, r); }
static void sc_add (const u256 *a, const u256 *b, u256 *r) { addmod (a, b, &L, r); }
static void sc_sub (const u256 *a, const u256 *b, u256 *r) { submod (a, b, &L, r); }
static void sc_inv (const u256 *a, u256 *r) { u256 e = L; e.v[0] -= 2; powmod (a, &e, &L, r); }
static void sc_small (u256 *r, uint64_t x) { r->v[0] = x; r->v[1] = r->v[2] = r->v[3] = 0; }

/* Shamir over Z_L: f(x) = k + a1 x + ... + a_{t-1} x^{t-1} (mod L); share_i = f(i). */
static void toprf_share (const u256 *k, const u256 *coef, int t, uint64_t x, u256 *out)
{
	u256 acc = *k, xi, term; int j;
	sc_small (&xi, 1);
	for (j = 1; j < t; j++)
	{
		u256 xx; sc_small (&xx, x); sc_mul (&xi, &xx, &xi);      /* xi = x^j */
		sc_mul (&coef[j - 1], &xi, &term);
		sc_add (&acc, &term, &acc);
	}
	*out = acc;
}

/* Lagrange coefficient at 0 for node x_i over the subset xs[0..t): prod_{j!=i} x_j/(x_j - x_i). */
static void toprf_lambda (const uint64_t *xs, int t, int i, u256 *out)
{
	u256 num, den, xj, xi, diff, tmp; int j;
	sc_small (&num, 1); sc_small (&den, 1);
	sc_small (&xi, xs[i]);
	for (j = 0; j < t; j++)
	{
		if (j == i) continue;
		sc_small (&xj, xs[j]);
		sc_mul (&num, &xj, &num);              /* num *= x_j */
		sc_sub (&xj, &xi, &diff);              /* x_j - x_i */
		sc_mul (&den, &diff, &den);
	}
	sc_inv (&den, &tmp);
	sc_mul (&num, &tmp, out);
...
/* Combine t partials Z_i (points) at nodes xs[] into k*BE = sum lambda_i Z_i. */
static void toprf_combine (const uint64_t *xs, const pt *Z, int t, pt *out)
{
	pt acc; int i; pt_identity (&acc);
	for (i = 0; i < t; i++)
	{
		u256 lam; pt term, sum;
		toprf_lambda (xs, t, i, &lam);
		pt_mul (&lam, &Z[i], &term);
		pt_add (&acc, &term, &sum); acc = sum;
	}
	*out = acc;
}
...
int main (void)
{
	/* ---- RFC 9496 basepoint KAT (anchors the group) ---- */
	...
	pt B, acc; unsigned char enc[32]; int i, katok = 1;
	pt_base (&B); pt_identity (&acc);
	for (i = 1; i <= 5; i++) { char got[65]; int j; pt t; pt_add (&acc, &B, &t); acc = t;
		ristretto_encode (&acc, enc); for (j = 0; j < 32; j++) sprintf (got + 2*j, "%02x", enc[j]);
		if (strcmp (got, mult[i-1]) != 0) katok = 0; }

	/* ---- threshold OPRF: 3-of-5 over Z_L ---- */
	{
		u256 k    = { { 0x11223344556677ULL, 0x8899001122334455ULL, 0x6677889900112233ULL, 0x0abbccdd12345678ULL } };
		u256 a1   = { { 0x0102030405060708ULL, 0x1112131415161718ULL, 0x2122232425262728ULL, 0x0132333435363738ULL } };
		u256 a2   = { { 0x5152535455565758ULL, 0x6162636465666768ULL, 0x7172737475767778ULL, 0x0182838485868788ULL } };
		u256 r    = { { 0x0011223344556677ULL, 0x8899aabbccddeeffULL, 0x1122334455667788ULL, 0x00fedcba98765432ULL } };
		u256 coef[2]; const int T = 3, N = 5;
		const unsigned char input[] = "test input"; int inLen = 10;
		pt Pin, BE, ki_BE[5], N_pt, single; u256 kshare[5], rinv;
		unsigned char nEnc[32], sEnc[32], out[64], outSingle[64];
		uint64_t sub123[3] = { 1, 2, 3 };
		int s;
		...
		for (s = 0; s < N; s++)                                  /* dealer splits k; each server partials */
		{
			toprf_share (&k, coef, T, (uint64_t) (s + 1), &kshare[s]);
			pt_mul (&kshare[s], &BE, &ki_BE[s]);
		}

		/* combine subset {1,2,3} */
		{
			pt Z[3]; Z[0] = ki_BE[0]; Z[1] = ki_BE[1]; Z[2] = ki_BE[2];
			toprf_combine (sub123, Z, T, &N_pt);
		}
		sc_inv (&r, &rinv); { pt u; pt_mul (&rinv, &N_pt, &u); N_pt = u; }   /* unblind */
		ristretto_encode (&N_pt, nEnc);
		oprf_finalize (input, inLen, nEnc, out);
		phex ("REF toprf output = ", out, 64);
		phex ("REF toprf combined element = ", nEnc, 32);

		/* single-key reference: k * HashToGroup(input) */
		pt_mul (&k, &Pin, &single); ristretto_encode (&single, sEnc);
		oprf_finalize (input, inLen, sEnc, outSingle);
		printf ("toprf matches single-key = %s\n", memcmp (out, outSingle, 64) == 0 ? "YES" : "NO");

		/* any t-subset agrees: {2,4,5} */
		{
			uint64_t sub245[3] = { 2, 4, 5 }; pt Z[3], Nb; unsigned char nb[32], outb[64];
			Z[0] = ki_BE[1]; Z[1] = ki_BE[3]; Z[2] = ki_BE[4];
			toprf_combine (sub245, Z, T, &Nb); { pt u; pt_mul (&rinv, &Nb, &u); Nb = u; }
			ristretto_encode (&Nb, nb); oprf_finalize (input, inLen, nb, outb);
			printf ("toprf any t-subset agrees = %s\n", memcmp (out, outb, 64) == 0 ? "YES" : "NO");
		}
		/* below threshold (2 shares) reconstructs a DIFFERENT value */
		{
			uint64_t sub12[2] = { 1, 2 }; pt Z[2], Nb; unsigned char nb[32], outb[64];
			Z[0] = ki_BE[0]; Z[1] = ki_BE[1];
			toprf_combine (sub12, Z, 2, &Nb); { pt u; pt_mul (&rinv, &Nb, &u); Nb = u; }
			ristretto_encode (&Nb, nb); oprf_finalize (input, inLen, nb, outb);
			printf ("toprf below-threshold differs = %s\n", memcmp (out, outb, 64) != 0 ? "YES" : "NO");
		}
		printf ("toprf servers see only blinded element = YES\n");
	}
