...
echo ""
echo "[103] Adiantum EncryptionMode SHIM (src/Volume/EncryptionModeAdiantum) — the wide-block volume path"
# Step [91] proves the Adiantum ALGORITHM against all 18 official google/adiantum KATs. It says nothing
# about the EncryptionMode subclass, which did not exist — V2FormatBinding.h recorded itself as "BLOCKED
# ON the wide-block cipher mode classes" and the Adiantum row in docs/CANT-CLAIMS-AUDIT.md was a code
# task for exactly that reason. This is a PROPERTY test of the shim (integration faults the KATs cannot
# see: tweak convention, SectorOffset, sector/unit confusion, in-place aliasing). The load-bearing one is
# WIDE-BLOCK DIFFUSION — flip one plaintext bit and the WHOLE sector must change (XTS changes 16 bytes);
# that is the entire reason to prefer Adiantum here, and it is what a bad shim silently loses.
AM_CC=""; for c in clang++ g++ c++; do if command -v "$c" >/dev/null 2>&1; then AM_CC="$c"; break; fi; done
AM_D="-DVC_ENABLE_ADIANTUM -DVC_ENABLE_ADIANTUM_MODE -DVC_ENABLE_CTAES -DVC_ENABLE_POLY1305"
AM_NOASM="-DCRYPTOPP_DISABLE_ASM -DCRYPTOPP_DISABLE_SSE2 -DCRYPTOPP_DISABLE_SSSE3"
AM_INC="$INC -I$SRCROOT/Crypto -I$SRCROOT/PKCS11 -I$SRCROOT/Crypto/Argon2/include"
AM_ARCH="$SRCROOT/Volume/Volume.a $SRCROOT/Core/Core.a $SRCROOT/Platform/Platform.a"
if [ -n "$AM_CC" ] && [ -f "$SRCROOT/Volume/Volume.a" ] && [ -f "$SRCROOT/Platform/Platform.a" ]; then
	am_ok=1
	for f in Adiantum AesCt chacha256 Poly1305; do
		"${AM_CC%%+*}" -O2 $AM_D $AM_NOASM $AM_INC -c "$SRCROOT/Crypto/$f.c" -o "/tmp/am_$f.o" 2>>/tmp/am.log || am_ok=0
	done
	"$AM_CC" -O2 -std=c++17 $AM_D $AM_NOASM -DTC_UNIX -DTC_LINUX -D_FILE_OFFSET_BITS=64 -DARGON2_NO_THREADS \
		$AM_INC -c "$HERE/adiantum_mode_test.cpp" -o /tmp/am_test.o 2>>/tmp/am.log || am_ok=0
	"$AM_CC" -O2 -std=c++17 $AM_D $AM_NOASM -DTC_UNIX -DTC_LINUX -D_FILE_OFFSET_BITS=64 -DARGON2_NO_THREADS \
		$AM_INC -c "$SRCROOT/Volume/EncryptionModeAdiantum.cpp" -o /tmp/am_mode.o 2>>/tmp/am.log || am_ok=0
	if [ "$am_ok" = 1 ] && "$AM_CC" -o /tmp/am_run /tmp/am_test.o /tmp/am_mode.o /tmp/am_Adiantum.o \
			/tmp/am_AesCt.o /tmp/am_chacha256.o /tmp/am_Poly1305.o \
			-Wl,--start-group $AM_ARCH -Wl,--end-group \
			$(pkg-config fuse --libs 2>/dev/null) $(wx-config --libs base 2>/dev/null) -lpthread 2>>/tmp/am.log; then
		/tmp/am_run > /tmp/am_out.txt 2>&1; amrc=$?
		sed 's/^/  /' /tmp/am_out.txt
		if [ "$amrc" = 0 ] && grep -q 'ADIANTUM MODE SHIM TEST PASSED' /tmp/am_out.txt; then
			echo "    MATCH: the shim is a true wide-block mode (1-bit change randomises the whole sector)"
		else echo "    ADIANTUM MODE SHIM TEST FAILED"; exit 1; fi
	else
		skip_step " could not build the Adiantum mode test (needs product archives; see /tmp/am.log)"
	fi
else
	skip_step " no C++ compiler or no product archives for the Adiantum mode test (build with ADIANTUM_MODE=1)"
fi
