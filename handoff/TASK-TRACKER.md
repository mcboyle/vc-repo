...
| ID | Task | Source | Status | Notes |
|---|---|---|---|---|
| **T2-1** | Bind volume salt in **HKF-v2** HKDF-Extract per Rank-1 | D-1 | **DONE (code)** | Gated `VC_ENABLE_HKF_MIX_V2_SALTBIND` (`make HKF_MIX_V2_SALTBIND=1`): HKDF-Extract salt becomes the volume salt (was `0^32`). New `HKFMixResponseIntoPasswordV2Salt` / `…VerSalt` thread the salt through `HKFApplyIfConfiguredVer` (C create), `Volumes.c` (C mount), and the C++ mount/create overload (`HKFMixPasswordWithResponse`←`VolumeHeader::Decrypt`/`HKFMixPasswordVer`). Proven two ways at step `[93]` (`hkf_saltbind_test.c` + `hkf_mixv2_reference.py`): salt-bound == python HKDF(salt=volume salt); unbound byte-identical to step-`[80]` anchor; different salts → different keys; NULL salt falls back. Closes R-2. Suite 92/92. Migration (existing v2 re-derive) + real-build mount/create round-trip = **T1-3**. |
| **T2-2** | Recovery-share encoding: **codex32 default + bech32m ≤ 89 chars**; add BIP-173 insertion-deletion note to `docs/VSS-SPEC.md` | D-2 | **DONE** | **T2-2a:** the `vcs1` short code upgraded bech32→**bech32m** (BIP-350, const `0x2bc830a3`) in `Common/ShareCode.c`; 89-char guarantee window is now the written constant `SHARECODE_BECH32M_MAX_CHARS`; step `[42]` re-anchored to the official `bech32m("a",empty)==a1lqfn3a`. **T2-2b:** **codex32** (BIP-93) added as the default export — `ShareCodeCodex32Encode/Decode` with the ms32 error-correcting checksum (13-symbol regular + 15-symbol long, portable two-word 65/76-bit polymod), proven against the official BIP-93 vectors (decode of the 128-bit `test` + 512-bit `0C8V` secrets recovers their exact published seeds) + independent python (step `[92]`, `codex32_test.c`/`codex32_reference.py`). **T2-2c:** BIP-173 insertion/deletion note in `docs/VSS-SPEC.md §3`. 89 = written constant. Closes R-3. Suite 91/91, 0 skipped. |
...
| **T2-4** | Promote **HCTR2 + Adiantum** into `src/`, both on every platform | D-4 | **IN PROGRESS** | **T2-4a done:** constant-time AES-256 **encrypt+decrypt** promoted to shippable `src/Crypto/AesCt.{c,h}` (gated `-DVC_ENABLE_CTAES` / `make CTAES=1`; step `[88]`, real AesCt.o vs FIPS-197 + Gladman 4096 blocks both dirs + Dec(Enc(x))==x + ctgrind CLEAN). **T2-4b done:** the real `src/Crypto/AesCt` substituted as Adiantum's block cipher (`adiantum_poc.c -DADIANTUM_USE_CTAES`) reproduces **all 19 official google/adiantum KAT lines** byte-for-byte (== table Gladman AES; step `[89]`) — constant-time AES is a proven drop-in for the non-AES-NI wide-block path; default Gladman build unchanged (step `[24]` unaffected). **T2-4c done:** Poly1305 promoted to shippable `src/Crypto/Poly1305.{c,h}` (gated `-DVC_ENABLE_POLY1305` / `make POLY1305=1`; step `[90]`, real Poly1305.o vs RFC 8439 §2.5.2/A.3 + reference over 4096 inputs). **T2-4d done:** the Adiantum mode itself promoted to shippable `src/Crypto/Adiantum.{c,h}` (gated `-DVC_ENABLE_ADIANTUM` / `make ADIANTUM=1`, implies CTAES+POLY1305; step `[91]`, real Adiantum.o linked against real AesCt.o + chacha256.o + Poly1305.o reproduces every official google/adiantum vector both directions == python, + whole-sector diffusion / wrong-key / wrong-tweak / in==out aliasing / bounds guard). Remaining: the C++ `EncryptionModeAdiantum`/HCTR2 classes calling these per sector on the non-AES-NI path (real-build C++). |
