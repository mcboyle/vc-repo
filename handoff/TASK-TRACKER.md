...
| **T2-4** | Promote **HCTR2 + Adiantum** into `src/`, both on every platform | D-4 | **IN PROGRESS** | **T2-4a done:** constant-time AES-256 **encrypt+decrypt** promoted to shippable `src/Crypto/AesCt.{c,h}` (gated `-DVC_ENABLE_CTAES` / `make CTAES=1`; step `[88]`, real AesCt.o vs FIPS-197 + Gladman 4096 blocks both dirs + Dec(Enc(x))==x + ctgrind CLEAN). **T2-4b done:** the real `src/Crypto/AesCt` substituted as Adiantum's block cipher (`adiantum_poc.c -DADIANTUM_USE_CTAES`) reproduces **all 19 official google/adiantum KAT lines** byte-for-byte (== table Gladman AES; step `[89]`) — constant-time AES is a proven drop-in for the non-AES-NI wide-block path; default Gladman build unchanged (step `[24]` unaffected). Remaining: HCTR2/Adiantum `EncryptionMode` classes calling AesCt on the non-AES-NI path (real-build C++). |
