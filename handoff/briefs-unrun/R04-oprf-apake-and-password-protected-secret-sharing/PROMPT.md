# R4 — OPRF, aPAKE and password-protected secret sharing
...
Explain the current state of OPAQUE and the aPAKE family, the CFRG prime-order-group OPRF
specification, verifiable OPRFs (VOPRF/POPRF), and password-protected secret sharing (PPSS, e.g.
JKKX14). My goal: make offline brute force against a stolen disk header impossible by requiring a
blinded online interaction with a server that never learns the password or the derived key, and is
rate-limited. Cover which construction actually fits a disk-unlock flow, what a server compromise
yields, the precise offline/online security distinction, how to handle an unreachable server without
destroying either security or availability, and what metadata the server unavoidably learns
(timing, frequency, IP). Is this meaningfully better than a hardware token for my threat model?
